...
public final class SecurityUtils {
	...
	public static Optional<String> getCurrentUserUuid() {
		...
		return Optional.ofNullable(securityContext.getAuthentication()).map(authentication -> {
			if (authentication.getPrincipal() instanceof UserDetails) {
				...
				return springSecurityUser.getUserUuid();
			} else if (authentication.getPrincipal() instanceof String) {
				return (String) authentication.getPrincipal();
			}
			return null;
	...
	public static String getCurrentUserRole() {
		...
		return customUserDetails.getRoles().stream().map(mapper->mapper.getName()).collect(Collectors.joining(","));
	}
	
	
	public static void getCurrentUserPermittedApi() {
		SecurityContext securityContext = SecurityContextHolder.getContext();
		CustomUserDetails customUserDetails= (CustomUserDetails)securityContext.getAuthentication().getPrincipal();
		 System.out.println(customUserDetails.getAuthApi());
	}
}
